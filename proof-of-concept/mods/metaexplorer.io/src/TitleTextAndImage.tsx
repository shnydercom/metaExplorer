...
export var TitleTextAndImageName: string =
	"metaexplorer.io/TitleTextAndContainer";
...
@ldBlueprint(bpCfg)
export class PureTitleTextAndImage
	extends Component<
		LDConnectedState & LDConnectedDispatch & LDOwnProps,
		LDLocalState & TitleTextAndImageState
	>
	...
	protected debouncedDirection = debounce((width) => {
		let directionChangeBreakPoint = this.state.localValues.get(
			VisualKeysDict.directionChangeBreakPoint
		);
	...
	render() {
		...
		const switchVertical = localValues.get(
			VisualKeysDict.switchVerticalDirection
		);
		const switchHorizontal = localValues.get(
			VisualKeysDict.switchHorizontalDirection
		);
		...
		return (
			<div
				className={`flex-container ${CSS_BASECLASS} ${switcherCSSclassName}`}
				ref={(divElement) => {
			...
			>
				<div
					className={`flex-filler vh-centered-column ${CSS_BASECLASS}-inputcontainer`}
					style={{ minHeight: DEFAULT_BREAKPOINT }}
				>
					{this.renderSub(VisualKeysDict.inputContainer)}
				</div>
				<div
					className={`flex-filler vh-centered-column ${CSS_BASECLASS}-textcontainer`}
					style={{ minHeight: "300px" }}
				>
					<h2 className={`${CSS_BASECLASS}-heading`}>
						{headerText ? headerText : ""}
					</h2>
					<p className={`${CSS_BASECLASS}-text`}>
